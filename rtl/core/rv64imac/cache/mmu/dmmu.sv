...
//sram interface
logic [63:0]             tlb_4K_valid;
logic [127:0]            tlb_4K[0:63];
logic [63:0]             tlb_4K_wen;
logic [15:0]             tlb_sp_valid;
logic [127:0]            tlb_sp[0:15];
logic [15:0]             tlb_sp_wen;
logic [32:0]             tlb_hit;
...
logic [1:0]              stage_status;
logic                    dmmu_miss_valid_reg;
logic [63:0]             paddr_out;
logic                    paddr_error_out;
//跳过mmu阶段
logic [1:0]              use_priv;
logic                    stage_jump_mmu;
...
assign stage_jump_mmu       =   (use_priv == M_LEVEL) | (satp_mode == 4'h0);
...
assign page_wen         = (stage_status == WAIT_RESP) & (pte_valid) & (!pte_error);
//**********************************************************************************************
//?output
assign dmmu_miss_valid  = dmmu_miss_valid_reg;
assign vaddr_d          = vaddr[63:0];
assign mmu_fifo_ready   = ((stage_jump_mmu | (|tlb_hit) | ((stage_status == WAIT_RESP) & pte_valid) | (vaddr[63:39] != {25{vaddr[38]}})) & mmu_fifo_valid & (paddr_ready | (!paddr_valid)));
FF_D_with_syn_rst #(
    .DATA_LEN 	(1  ),
    .RST_DATA 	(0  ))
u_paddr_valid(
...
assign paddr_error_out  =   ((vaddr[63:39] != {25{vaddr[38]}}) | 
                            ...
                            ((stage_status == WAIT_RESP) & pte_valid & pte_error)) & (!stage_jump_mmu);
endmodule //dmmu
