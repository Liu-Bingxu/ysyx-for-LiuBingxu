...
//interface with immu
    ...
    input  [63:0]           vaddr_i,
//interface with store_dmmu
    input                   store_dmmu_miss_valid,
    output                  store_dmmu_miss_ready,
    input  [63:0]           store_vaddr_d,
//interface with load_dmmu
    input                   load_dmmu_miss_valid,
    output                  load_dmmu_miss_ready,
    input  [63:0]           load_vaddr_d,
...
generate
    for(tlb_group_index = 0; tlb_group_index < MMU_GROUP; tlb_group_index = tlb_group_index + 1)begin: tlb_group_sram
        for(tlb_way_index = 0; tlb_way_index < MMU_WAY; tlb_way_index = tlb_way_index + 1)begin: tlb_way_sram
            ...
            if(MMU_GROUP == 1)begin : U_gen_sram_wen_only_one_group
                assign sram_wen[tlb_group_index][tlb_way_index] = (!tlb_page_wen) | (tlb_way_index != rand_way);
                if(tlb_group_index == 0)begin : U_gen_tlb_rdata_from_group
            ...
            else begin : U_gen_sram_wen_more_group
            ...
            if(tlb_group_index == 0)begin : U_sprit_msg_from_tlb_rdata
        ...
        if(MMU_GROUP == 1)begin : U_gen_sram_cen_only_one_group
            assign sram_cen[tlb_group_index]                     = (!immu_miss_valid) & (!store_dmmu_miss_valid) & (!load_dmmu_miss_valid) & (stage_status == IDLE);
        end
        else begin : U_gen_sram_cen_more_group
            assign sram_cen[tlb_group_index]                     = ((!(immu_miss_valid & immu_miss_ready)) | (tlb_group_index != vaddr_i[17 + MMU_GROUP_LEN:18])) & 
                                                                    ((!(store_dmmu_miss_valid & store_dmmu_miss_ready)) | (tlb_group_index != store_vaddr_d[17 + MMU_GROUP_LEN:18])) & 
                                                                    ((!(load_dmmu_miss_valid & load_dmmu_miss_ready)) | (tlb_group_index != load_vaddr_d[17 + MMU_GROUP_LEN:18])) & 
...
assign sram_1G_addr = ((stage_status != IDLE) ? stage_vaddr[35:30] : (immu_miss_valid) ? vaddr_i[35:30] : ((store_dmmu_miss_valid) ? store_vaddr_d[35:30] : load_vaddr_d[35:30]));
...
assign sram_2M_addr = ((stage_status != IDLE) ? stage_vaddr[26:21] : (immu_miss_valid) ? vaddr_i[26:21] : ((store_dmmu_miss_valid) ? store_vaddr_d[26:21] : load_vaddr_d[26:21]));
...
assign sram_sp_cen = (!immu_miss_valid) & (!store_dmmu_miss_valid) & (!load_dmmu_miss_valid) & (stage_status == IDLE);
rand_lfsr_8_bit #(
...
assign sram_addr                    = ((stage_status != IDLE) ? stage_vaddr[17:12] : (immu_miss_valid) ? vaddr_i[17:12] : ((store_dmmu_miss_valid) ? store_vaddr_d[17:12] : load_vaddr_d[17:12]));
    ...
    if(!rst_n)begin
        ...
        case (stage_status)
            IDLE: begin
                ...
                else if(store_dmmu_miss_valid & store_dmmu_miss_ready & (store_vaddr_d[63:39] != {25{store_vaddr_d[38]}}))begin
                    stage_status <= OUT;
                    out_error    <= 1'b1;
                end
                else if(store_dmmu_miss_valid & store_dmmu_miss_ready)begin
                    stage_status <= SEARCH_TLB;
                    stage_vaddr  <= store_vaddr_d;
                end
                else if(load_dmmu_miss_valid & load_dmmu_miss_ready & (load_vaddr_d[63:39] != {25{load_vaddr_d[38]}}))begin
                    stage_status <= OUT;
                    out_error    <= 1'b1;
                end
                else if(load_dmmu_miss_valid & load_dmmu_miss_ready)begin
                    stage_status <= SEARCH_TLB;
                    stage_vaddr  <= load_vaddr_d;
                end
            end
            SEARCH_TLB: begin
