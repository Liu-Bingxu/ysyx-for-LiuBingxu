...
iq_acc_entry_t                 issue_queue_not_alu;
iq_ptr_t                       issue_queue_not_alu_ptr;
logic         [IQ_NUM - 1 : 0] iq_can_issue_not_alu;
logic         [IQ_NUM - 1 : 0] iq_can_issue_not_alu_valid;
iq_ptr_t      [IQ_NUM - 1 : 0] iq_can_issue_not_alu_ptr;
...
logic iq_enq_num_dec_not_alu;
assign iq_enq_num_inc           = in_valid & in_ready;
assign iq_enq_num_dec_alu       = out1_valid & out1_ready;
assign iq_enq_num_dec_not_alu   = out2_valid & out2_ready;

logic                   iq_enq_num_wen;
logic [IQ_W - 1 : 0]    iq_enq_num_nxt;
assign iq_enq_num_wen           = (iq_enq_num_inc |  iq_enq_num_dec_alu | iq_enq_num_dec_not_alu);
assign iq_enq_num_nxt           = (iq_enq_num + {{(IQ_W - 1){1'b0}}, iq_enq_num_inc} - {{(IQ_W - 1){1'b0}}, iq_enq_num_dec_alu} - {{(IQ_W - 1){1'b0}}, iq_enq_num_dec_not_alu});
FF_D_with_syn_rst #(
    .DATA_LEN 	( IQ_W  ),
    .RST_DATA 	( 0     )
)u_iq_enq_num
...
generate for(iq_index = 0 ; iq_index < IQ_NUM; iq_index = iq_index + 1) begin : U_gen_issue_queue 
    ...
    assign iq_can_issue_not_alu [iq_index] = issue_queue_valid[iq_index] & (issue_queue[iq_index].futype != fu_alu) & 
                                        (issue_queue[iq_index].src1_status == reg_status_fire) & (issue_queue[iq_index].src2_status == reg_status_fire);
    if(iq_index == 0)begin : u_gen_iq_ptr_0
        ...
        assign iq_can_issue_not_alu_valid [iq_index] = iq_can_issue_not_alu [iq_index];
        assign iq_can_issue_not_alu_ptr   [iq_index] = iq_index;
    end
        ...
        assign iq_can_issue_not_alu_valid [iq_index] = (iq_can_issue_not_alu [iq_index] | iq_can_issue_not_alu_valid [iq_index - 1]);
        assign iq_can_issue_not_alu_ptr   [iq_index] = (iq_can_issue_not_alu_valid [iq_index - 1]) ? iq_can_issue_not_alu_ptr[iq_index - 1] : iq_index;
    end
    ...
    logic dequeue;
    logic dequeue_alu;
    logic dequeue_not_alu;
    assign enqueue          = in_valid & in_ready & (iq_index == issue_queue_enq_ptr);
    assign dequeue          = (dequeue_alu | dequeue_not_alu);
    assign dequeue_alu      = out1_valid & out1_ready & (iq_index == issue_queue_alu_ptr);
    assign dequeue_not_alu  = out2_valid & out2_ready & (iq_index == issue_queue_not_alu_ptr);
    FF_D_with_syn_rst #(
        .DATA_LEN 	( 1  ),
        .RST_DATA 	( 0  )
    )u_issue_queue_valid
    ...
    iq_acc_entry_t                 issue_queue_src1_fire_entry;
    logic                          issue_queue_src2_fire;
    iq_acc_entry_t                 issue_queue_src2_fire_entry;
    logic                          issue_queue_entry_wen;
    iq_acc_entry_t                 issue_queue_entry_nxt;
    ...
    assign issue_queue_entry_nxt =  ({IQ_ACC_ENTRY_W{enqueue                }} & issue_queue_enq            ) | 
                                    ({IQ_ACC_ENTRY_W{issue_queue_src1_fire  }} & issue_queue_src1_fire_entry) | 
                                    ({IQ_ACC_ENTRY_W{issue_queue_src2_fire  }} & issue_queue_src2_fire_entry);
    FF_D_without_asyn_rst #(IQ_ACC_ENTRY_W)u_entry     (clk,issue_queue_entry_wen, issue_queue_entry_nxt, issue_queue[iq_index]);
end
endgenerate
...
assign issue_queue_not_alu_ptr  = iq_can_issue_not_alu_ptr[IQ_NUM - 1];

assign issue_queue_alu          = issue_queue[issue_queue_alu_ptr];
assign issue_queue_not_alu      = issue_queue[issue_queue_not_alu_ptr];
...
assign out2_valid               = iq_can_issue_not_alu_valid[IQ_NUM - 1];
assign out2.fuoptype            = issue_queue_not_alu.fuoptype ;
assign out2.psrc1               = issue_queue_not_alu.psrc1    ;
assign out2.src1_type           = issue_queue_not_alu.src1_type;
assign out2.psrc2               = issue_queue_not_alu.psrc2    ;
assign out2.src2_type           = issue_queue_not_alu.src2_type;
assign out2.pwdest              = issue_queue_not_alu.pwdest   ;
assign out2.imm                 = issue_queue_not_alu.imm      ;
assign out2.rob_ptr             = issue_queue_not_alu.rob_ptr  ;
