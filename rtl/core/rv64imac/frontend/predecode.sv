...
module predecode(
    ...
    output [IFU_INST_MAX_NUM * 1 - 1 : 0]   o_is_rvc,
    output [32 * IFU_INST_MAX_NUM - 1:0]    o_inst,
    output [16 * IFU_INST_MAX_NUM - 1:0]    o_c_inst,
...
generate for(packed_index = 0 ; packed_index < IFU_INST_MAX_NUM; packed_index = packed_index + 1) begin : U_gen_packed_index
    ...
    assign o_is_rvc[packed_index]        = is_rvc    [packed_index];
    assign o_inst[32 * packed_index + 31 : 32 * packed_index]        = inst      [packed_index];
    assign o_c_inst[16 * packed_index + 15 : 16 * packed_index]      = c_inst    [packed_index];
...
logic  [31:0]                   inst_16_32[IFU_INST_MAX_NUM -1 :0];
logic  [31:0]                   decode_inst[IFU_INST_MAX_NUM -1 :0];
...
generate for(inst_index = 0 ; inst_index < IFU_INST_MAX_NUM; inst_index = inst_index + 1) begin : U_gen_inst_index_decode
    ...
    inst16_to_32 u_inst16_to_32(
        .input_inst  	( predecode_inst[inst_index][15:0]  ),
        .output_inst 	( inst_16_32[inst_index]            )
    );

    assign decode_inst[inst_index]  = (inst_is_rvc[inst_index]) ? inst_16_32[inst_index] : predecode_inst[inst_index];

    assign funct3[inst_index]       = decode_inst[inst_index][14:12];
    assign B_flag[inst_index]       = (decode_inst[inst_index][6:0]==7'b1100011)?1'b1:1'b0;

    assign jal[inst_index]          =   (decode_inst[inst_index][6:0]                        ==      7'b1101111  ) ? 1'b1 : 1'b0;
    assign jalr[inst_index]         =   ({funct3[inst_index],decode_inst[inst_index][6:0]}   ==  10'b0001100111  ) ? 1'b1 : 1'b0;

    assign beq[inst_index]          =   (B_flag[inst_index]&(funct3[inst_index]==3'b000))?1'b1:1'b0;
    assign bne[inst_index]          =   (B_flag[inst_index]&(funct3[inst_index]==3'b001))?1'b1:1'b0;
    assign blt[inst_index]          =   (B_flag[inst_index]&(funct3[inst_index]==3'b100))?1'b1:1'b0;
    assign bge[inst_index]          =   (B_flag[inst_index]&(funct3[inst_index]==3'b101))?1'b1:1'b0;
    assign bltu[inst_index]         =   (B_flag[inst_index]&(funct3[inst_index]==3'b110))?1'b1:1'b0;
    assign bgeu[inst_index]         =   (B_flag[inst_index]&(funct3[inst_index]==3'b111))?1'b1:1'b0;

    assign jal_call[inst_index]     =   (jal[inst_index]  & (decode_inst[inst_index][11:7] == 5'h1));
    assign jalr_call[inst_index]    =   (jalr[inst_index] & (decode_inst[inst_index][11:7] == 5'h1));
    assign jalr_ret[inst_index]     =   (decode_inst[inst_index]       == 32'h8067);

    assign imm_I[inst_index] = {{(52){decode_inst[inst_index][31]}},decode_inst[inst_index][31:20]};
    assign imm_B[inst_index] = {{(52){decode_inst[inst_index][31]}},decode_inst[inst_index][7],decode_inst[inst_index][30:25],decode_inst[inst_index][11:8],1'b0};
    assign imm_J[inst_index] = {{(44){decode_inst[inst_index][31]}},decode_inst[inst_index][19:12],decode_inst[inst_index][20],decode_inst[inst_index][30:21],1'b0};

    assign imm[inst_index] = (jalr[inst_index])?imm_I[inst_index]:(
        (jal[inst_index])?imm_J[inst_index]:imm_B[inst_index]
    );
    ...
    assign inst_is_bracnch[inst_index]      = (beq[inst_index] | bne[inst_index] | blt[inst_index] | bge[inst_index] | bltu[inst_index] | bgeu[inst_index]);
    assign inst_is_jal[inst_index]          = ((jal[inst_index] & (!jal_call[inst_index])));
    assign inst_is_call[inst_index]         = (jal_call[inst_index] | jalr_call[inst_index]);
    assign inst_is_ret[inst_index]          = jalr_ret[inst_index];
    assign inst_is_jalr[inst_index]         = (jalr[inst_index] & (!jalr_call[inst_index]) & (!jalr_ret[inst_index]));
    ...
    assign inst_inst[inst_index]            = decode_inst[inst_index];
    ...
    assign c_inst[inst_index]          = predecode_inst[inst_index][15:0]   ;
